...
{
//   printf(" bgt v: %d c: %d n: %d z: %d\r\n",
//          cc.bit.v, cc.bit.c, cc.bit.n, cc.bit.z);
//   printf("bgt, condition %d\r\n", !(cc.bit.z & (cc.bit.n ^ cc.bit.v)));
...
void mc6809::help_sub(Byte& x)
{
  ...
  DWord tx = x;
  DWord result = tx - v;
  cc.bit.v = ((v ^ tx ^ result ^ (result >> 1)) >> 7);
  cc.bit.z = !(result & 0xff);
  cc.bit.n = (result & 0x80) >> 7;
  cc.bit.c = (result & 0x100) >> 8;
  x = result & 0xff;
}
...
void mc6809::subd(void)
{
  ...
  DWord td = d;
  DWord result = td - v;
  cc.bit.v = ((v ^ td ^ result ^ (result >> 1)) >> 15);
  cc.bit.z = !(result & 0xffff);
  cc.bit.n = (result & 0x8000) >> 15;
  cc.bit.c = (result & 0x10000) >> 16;
  d = result & 0xffff;
}
